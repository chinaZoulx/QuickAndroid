...
public abstract class BaseListActivity2<M> extends ThemeActivity implements SwipeRefreshLayout.OnRefreshListener, XStickyListHeadersListView.OnRefreshListener {
    ...
    BaseListActivity.ErrorType errorType = BaseListActivity.ErrorType.NORMAL;
    ...
    public void onRefreshClick(BaseListActivity.ErrorType errorType) {
        onRefresh();
    }
    ...
    public void requestData(int pageNumber) {
        if (!CheckUtils.INSTANCE.isNetWorkAvailable()) {
            setHasData(false, BaseListActivity.ErrorType.NET_WORK);
            return;
        }
        params.clear();
    ...
    public abstract void onBindDataItemView(BaseViewHolder holder, M itemData, int position);

    public abstract void onBindDataHeaderView(BaseViewHolder holder, M itemData, int position);
    ...
    public class Adapter extends BaseAdapter implements StickyListHeadersAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            if (convertView == null) {
                holder = new BaseViewHolder(LayoutInflater.from(parent.getContext()).inflate(onResultItemLayout(), parent, false));
                holder.itemView.setTag(holder);
            } else holder = (BaseViewHolder) convertView.getTag();
        ...
        public View getHeaderView(int position, View convertView, ViewGroup parent) {
            BaseViewHolder holder;
            if (convertView == null) {
                holder = new BaseViewHolder(LayoutInflater.from(parent.getContext()).inflate(onResultHeaderLayout(), parent, false));
                holder.itemView.setTag(holder);
            } else holder = (BaseViewHolder) convertView.getTag();
