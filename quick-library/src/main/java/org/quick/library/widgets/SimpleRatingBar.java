...
public class SimpleRatingBar extends View {
    ...
    public enum Gravity {
        ...
        static Gravity fromId ( int id ) {
            ...
            // default value
            Log.w ( "SimpleRatingBar", String.format (
    ...
    /**
     * Inits paintCir objects and default values.
     */
    private void initView () {
        ...
        defaultStarSize = applyDimension ( COMPLEX_UNIT_DIP, 30,
    ...
    private void parseAttrs ( AttributeSet attrs ) {
        ...
        TypedArray arr = getContext ()
                .obtainStyledAttributes ( attrs, R.styleable.SimpleRatingBar );

        borderColor = arr.getColor ( R.styleable.SimpleRatingBar_srb_borderColor,
                                     getResources ().getColor ( R.color.colorStarsBorder ) );
        fillColor = arr.getColor ( R.styleable.SimpleRatingBar_srb_fillColor, borderColor );
        starBackgroundColor = arr.getColor ( R.styleable.SimpleRatingBar_srb_starBackgroundColor,
                                             Color.TRANSPARENT );
        backgroundColor = arr
                .getColor ( R.styleable.SimpleRatingBar_srb_backgroundColor, Color.TRANSPARENT );

        pressedBorderColor = arr
                .getColor ( R.styleable.SimpleRatingBar_srb_pressedBorderColor, borderColor );
        pressedFillColor = arr
                .getColor ( R.styleable.SimpleRatingBar_srb_pressedFillColor, fillColor );
        pressedStarBackgroundColor = arr
                .getColor ( R.styleable.SimpleRatingBar_srb_pressedStarBackgroundColor,
                            starBackgroundColor );
        pressedBackgroundColor = arr
                .getColor ( R.styleable.SimpleRatingBar_srb_pressedBackgroundColor,
                            backgroundColor );

        numberOfStars = arr.getInteger ( R.styleable.SimpleRatingBar_srb_numberOfStars, 5 );

        starsSeparation = arr
                .getDimensionPixelSize ( R.styleable.SimpleRatingBar_srb_starsSeparation,
                                         ( int ) valueToPixels ( 4, Dimension.DP ) );
        maxStarSize = arr.getDimensionPixelSize ( R.styleable.SimpleRatingBar_srb_maxStarSize,
                                                  Integer.MAX_VALUE );
        desiredStarSize = arr.getDimensionPixelSize ( R.styleable.SimpleRatingBar_srb_starSize,
                                                      Integer.MAX_VALUE );
        ...
        drawBorderEnabled = arr
                .getBoolean ( R.styleable.SimpleRatingBar_srb_drawBorderEnabled, true );
        gravity = Gravity
                .fromId ( arr.getInt ( R.styleable.SimpleRatingBar_srb_gravity, Gravity.Left.id ) );
    ...
    private void validateAttrs () {
        ...
        if ( desiredStarSize != Integer.MAX_VALUE && maxStarSize != Integer.MAX_VALUE &&
                desiredStarSize
                        > maxStarSize ) {
            Log.w ( "SimpleRatingBar", String.format (
    ...
    protected void onMeasure ( int widthMeasureSpec, int heightMeasureSpec ) {
        ...
        if ( widthMode == MeasureSpec.EXACTLY ) {
            ...
            if ( desiredStarSize != Integer.MAX_VALUE ) {
                // user specified activity specific star size, so there is activity desired width
                int desiredWidth = calculateTotalWidth ( desiredStarSize, numberOfStars,
                                                         starsSeparation, true );
                width = Math.min ( desiredWidth, widthSize );
            } else if ( maxStarSize != Integer.MAX_VALUE ) {
                // user specified activity max star size, so there is activity desired width
                int desiredWidth = calculateTotalWidth ( maxStarSize, numberOfStars,
                                                         starsSeparation, true );
                width = Math.min ( desiredWidth, widthSize );
            } else {
                // using defaults
                int desiredWidth = calculateTotalWidth ( defaultStarSize, numberOfStars,
                                                         starsSeparation, true );
                width = Math.min ( desiredWidth, widthSize );
            }
        } else {
        ...
        float tentativeStarSize = ( width - getPaddingLeft () - getPaddingRight () -
                starsSeparation * ( numberOfStars - 1 ) ) / numberOfStars;
        ...
        if ( heightMode == MeasureSpec.EXACTLY ) {
            ...
            if ( desiredStarSize != Integer.MAX_VALUE ) {
                // user specified activity specific star size, so there is activity desired width
                int desiredHeight = calculateTotalHeight ( desiredStarSize, numberOfStars,
                                                           starsSeparation, true );
                height = Math.min ( desiredHeight, heightSize );
            } else if ( maxStarSize != Integer.MAX_VALUE ) {
                // user specified activity max star size, so there is activity desired width
                int desiredHeight = calculateTotalHeight ( maxStarSize, numberOfStars,
                                                           starsSeparation, true );
                height = Math.min ( desiredHeight, heightSize );
            } else {
                // using defaults
                int desiredHeight = calculateTotalHeight ( tentativeStarSize, numberOfStars,
                                                           starsSeparation, true );
                height = Math.min ( desiredHeight, heightSize );
            }
        } else {
    ...
    private float calculateBestStarSize ( int width, int height ) {
        ...
        if ( maxStarSize != Integer.MAX_VALUE ) {
            float desiredTotalWidth = calculateTotalWidth ( maxStarSize, numberOfStars,
                                                            starsSeparation, true );
            float desiredTotalHeight = calculateTotalHeight ( maxStarSize, numberOfStars,
                                                              starsSeparation, true );
            if ( desiredTotalWidth >= width || desiredTotalHeight >= height ) {
                // we need to shrink the size of the stars
                float sizeBasedOnWidth = ( width - getPaddingLeft () - getPaddingRight () -
                        starsSeparation * ( numberOfStars - 1 ) ) / numberOfStars;
    ...
    private void performStarSizeAssociatedCalculations ( int width, int height ) {
        ...
        float totalStarsWidth = calculateTotalWidth ( currentStarSize, numberOfStars,
                                                      starsSeparation, false );
        float totalStarsHeight = calculateTotalHeight ( currentStarSize, numberOfStars,
                                                        starsSeparation, false );
        float startingX =
                ( width - getPaddingLeft () - getPaddingRight () ) / 2 - totalStarsWidth / 2 +
                        getPaddingLeft ();
        float startingY =
                ( height - getPaddingTop () - getPaddingBottom () ) / 2 - totalStarsHeight / 2 +
                        getPaddingTop ();
        starsDrawingSpace = new RectF ( startingX, startingY, startingX + totalStarsWidth,
                                        startingY + totalStarsHeight );
        float aux = starsDrawingSpace.width () * 0.05f;
        starsTouchSpace = new RectF ( starsDrawingSpace.left - aux, starsDrawingSpace.top,
                                      starsDrawingSpace.right + aux, starsDrawingSpace.bottom );
        ...
        starVertex = new float[] {
                ...
                tipHorizontalMargin + triangleSide , innerUpHorizontalMargin , half ,
                ...
                currentStarSize - bottomFromMargin , currentStarSize - tipVerticalMargin ,
                // bottom right
                half , currentStarSize - innerCenterVerticalMargin , bottomFromMargin ,
    ...
    private void drawFromLeftToRight ( Canvas internalCanvas ) {
        ...
        for ( int i = 0 ; i < numberOfStars ; i++ ) {
            if ( remainingTotalRating >= 1 ) {
                ...
                drawStar ( internalCanvas, startingX, startingY, remainingTotalRating,
                           Gravity.Left );
    ...
    /**
     * Draws activity star in the provided canvas.
     *
     * @param canvas
    ...
    private void drawStar ( Canvas canvas, float x, float y, float filled, Gravity gravity ) {
        ...
        // Note: below, currentStarSize*0.02f is activity minor correction so the user won't see activity VERTICAL black line in between the fill and empty color
        if ( gravity == Gravity.Left ) {
            // color star fill
            canvas.drawRect ( x, y, x + fill + currentStarSize * 0.02f, y + currentStarSize,
                              paintStarFill );
            // draw star background
            canvas.drawRect ( x + fill, y, x + currentStarSize, y + currentStarSize,
                              paintStarBackground );
        } else {
            // color star fill
            canvas.drawRect ( x + currentStarSize - ( fill + currentStarSize * 0.02f ), y,
                              x + currentStarSize, y + currentStarSize, paintStarFill );
            // draw star background
            canvas.drawRect ( x, y, x + currentStarSize - fill, y + currentStarSize,
                              paintStarBackground );
        }
    ...
    /**
     * Assigns activity rating to the touch event.
     *
     * @param x
     * @param y
     */
  ...
  /* ----------- GETTERS AND SETTERS ----------- */
    ...
    public void setRating ( float rating ) {
        ...
        if ( stepSize != Float.MAX_VALUE &&
                ( ratingAnimator == null || !ratingAnimator.isRunning () ) ) {
    ...
    public void setStarSize ( float starSize ) {
        ...
        if ( starSize != Integer.MAX_VALUE && maxStarSize != Integer.MAX_VALUE &&
    ...
    /**
     * Convenience method to convert activity value in the given dimension to pixels.
     *
     * @param value
     * @param dimen
     *
        ...
        switch ( dimen ) {
            case Dimension.DP:
                return applyDimension ( COMPLEX_UNIT_DIP, value,
                                        getResources ().getDisplayMetrics () );
            case Dimension.SP:
                return applyDimension ( COMPLEX_UNIT_SP, value,
                                        getResources ().getDisplayMetrics () );
            default:
                return value;
        }
    ...
    /**
     * Convenience method to convert activity value from pixels to the given dimension.
     *
     * @param value
     * @param dimen
     *
    ...
    /**
     * Returns activity new AnimationBuilder.
     *
     * @return
     */
    ...
    public interface OnRatingBarChangeListener {
         ...
         * while the user is dragging, only when the user finalizes activity rating by
         * lifting the touch.
         *
         * @param simpleRatingBar The RatingBar whose rating has changed.
         * @param rating          The current rating. This will be in the range
         *                        0..numStars.
         * @param fromUser        True if the rating change was initiated by activity user's
    ...
    public class AnimationBuilder {
         ...
         *
         * @param repeatCount must be activity positive value or ValueAnimator.INFINITE
         *
         * @return
         */
