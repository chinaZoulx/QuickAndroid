...
import org.chris.quick.tools.common.Md5Utils;
        ...
        OkHttpUtils.post().url(url).tag(tag).params(params).build().execute(new StringCallback() {
            ...
            public void onError(Call call, Exception e, int id) {
                ...
                if (isCheck && !checkActivityIsRunning(activity)) {
                    Log.e("网络请求", "依赖的Activity已销毁,取消任务");
                    return;
                }
...
//                }
            ...
            public void onAfter(int id) {
                Log.e("okHttpUtils", String.format("\tEND：%s URL：%s", DateUtils.INSTANCE.formatToStr(DateUtils.INSTANCE.getCurrentTimeInMillis(), DateUtils.INSTANCE.getFormatDefault() + ":SSS"), url));
                if (isCheck && !checkActivityIsRunning(activity)) {
                    Log.e("网络请求", "依赖的Activity已销毁,取消任务");
                    return;
                }
