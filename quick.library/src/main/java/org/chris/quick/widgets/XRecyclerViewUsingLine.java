...
public class XRecyclerViewUsingLine extends XRecyclerView {
    ...
    public XRecyclerViewUsingLine(Context context) {
        this(context, null);
    }

    public XRecyclerViewUsingLine(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public XRecyclerViewUsingLine(Context context, @Nullable AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        this.mContext = context;
        setupParams(context.obtainStyledAttributes(attrs, R.styleable.XRecyclerViewUsingLine));
        setupLayout();
        setupListener();
    }

    private void setupParams(TypedArray typedArray) {
        mDivider = typedArray.getDrawable(R.styleable.XRecyclerViewUsingLine_xDivider);
        mOrientation = typedArray.getInt(R.styleable.XRecyclerViewUsingLine_xDividerOrientation, VERTICAL);
        mDividerShowState = typedArray.getInt(R.styleable.XRecyclerViewUsingLine_xShowDividers, DividerItemDecoration.NONE);
        int tempSize = typedArray.getDimensionPixelSize(R.styleable.XRecyclerViewUsingLine_xDividerSize, 1);
        if (DimenUtils.isPxVal(typedArray.peekValue(R.styleable.XRecyclerViewUsingLine_xDividerSize)))
            mDividerSize = AutoUtils.getPercentHeightSize(tempSize);
        else
            mDividerSize = tempSize;

        int tempPadding = typedArray.getDimensionPixelOffset(R.styleable.XRecyclerViewUsingLine_xDividerPadding, AutoUtils.getPercentWidthSize(getContext().getResources().getDimensionPixelSize(R.dimen.borderWidth)));
        if (DimenUtils.isPxVal(typedArray.peekValue(R.styleable.XRecyclerViewUsingLine_xDividerPadding)))
