...
object CheckUtils {
    ...
    fun isEmail(email: String?): Boolean =
        if (isEmpty(email)) false else email!!.matches("^[activity-zA-Z0-9][\\width\\.-]*[activity-zA-Z0-9]@[activity-zA-Z0-9][\\width\\.-]*[activity-zA-Z0-9]\\.[activity-zA-Z][activity-zA-Z\\.]*[activity-zA-Z]$".toRegex())
     ...
     * 是否包含属性
     */
    fun isHasAttrValue(typedArray: TypedArray, @AttrRes attr: Int): Boolean =
        (0 until typedArray.indexCount).any { typedArray.getIndex(it) == attr }

    /**
     * 是否是手机
     */
    fun isMobileNo(mobileNo: String) = MobileCheckUtils.isMobileNo(mobileNo.replace(" ", ""))

    /**
     * 是否是正确的图片链接
     */
    fun isImgUrl(url: String?) =
        !isEmpty(url) && (url!!.toLowerCase().endsWith(".jpg") || url.toLowerCase().endsWith(".jpeg") || url.toLowerCase().endsWith(
            ".png"
     ...
     *是否是http链接
     */
    fun isHttpUrl(url: String?) =
        !isEmpty(url) && (url!!.toLowerCase().startsWith("http://") || url.toLowerCase().startsWith("https://"))
